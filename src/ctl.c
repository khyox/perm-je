...
ctl_arena_init(ctl_arena_stats_t *astats)
{
	...
	if (astats->bstats == NULL) {
		astats->bstats = (malloc_bin_stats_t *)temp_malloc(nbins *
		    sizeof(malloc_bin_stats_t));
		if (astats->bstats == NULL)
			return (true);
	}
	if (astats->lstats == NULL) {
		astats->lstats = (malloc_large_stats_t *)temp_malloc(nlclasses *
...
ctl_refresh(void)
{
	...
	memcpy(tarenas, parenas, sizeof(arena_t *) * narenas);
	for (i = 0; i < narenas; i++) {
		if (parenas[i] != NULL)
			ctl_stats.arenas[i].nthreads = parenas[i]->nthreads;
		else
...
ctl_init(void)
{
	...
	if (ctl_initialized == false) {
		 ...
		 */
		ctl_stats.arenas = (ctl_arena_stats_t *)temp_malloc(
    ...
    void *newp, size_t newlen)
{
	...
	if (newind != oldind) {
		...
		malloc_mutex_lock(&arenas_lock);
		if ((arena = parenas[newind]) == NULL)
			arena = arenas_extend(newind);
		parenas[oldind]->nthreads--;
		parenas[newind]->nthreads++;
		malloc_mutex_unlock(&arenas_lock);
		if (arena == NULL) {
			ret = EAGAIN;
			goto RETURN;
    ...
    size_t *oldlenp, void *newp, size_t newlen)
{
	...
	if (swap_enabled) {
		...
		/*
		 * swap_prezeroed is used by the swap code after a successfull
		 * call to chunk_swap_enable().
		 */
    ...
    void *newp, size_t newlen)
{
	...
	if (swap_enabled) {
		...
		/* Copy the fds array. */
		swap_fds = (int *)temp_malloc(nfds * sizeof(int));
		if (swap_fds == NULL) {
			ret = EFAULT;
			goto RETURN;
		}
