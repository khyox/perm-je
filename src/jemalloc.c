...
static plib_t plib_block;
plib_t *plib = &plib_block;
bool plib_initialized = false;
...
/* Allocate memory from temporary heap. */
void *
temp_malloc(size_t size)
{
	intptr_t csize, odd;
	void *ptr;

	/* TODO: add mutex when needed */
	assert(size != 0);
	csize = CACHELINE_CEILING(size);
	if ((odd = ((size_t)sbrk(0) & CACHELINE_MASK))) sbrk(odd);
	if ((ptr = sbrk(csize)) != (void *)-1) return(ptr);
	else return(NULL);
}
...
arenas_extend(unsigned ind)
{
	 ...
	 * by using parenas[0].  In practice, this is an extremely unlikely
	 * failure.
	 */
	...
	if (narenas > 1) {
		...
		for (i = 1; i < narenas; i++) {
			if (parenas[i] != NULL) {
				 ...
				 */
				if (parenas[i]->nthreads <
				    parenas[choose]->nthreads)
					choose = i;
		...
		if (parenas[choose] == 0 || first_null == narenas) {
			/*
...
malloc_init_hard(void)
{
	...
	if (malloc_initialized || malloc_initializer == pthread_self()) {
		 ...
		 * thread, and it is recursively allocating. The setting of
		 * malloc_initializer implies allocation is ready for self,
		 * others wait.
		 */
	...
	if (!swap_enabled && base_boot()) {
		malloc_mutex_unlock(&init_lock);
		return (true);
	}

	if (!swap_enabled && chunk_boot()) {
		malloc_mutex_unlock(&init_lock);
		return (true);
	}

	if (!swap_enabled && ctl_boot()) {
		malloc_mutex_unlock(&init_lock);
		return (true);
	}
	...
	/* BEGIN BLOCK (plib_initialized == false) */
	/*
	...
	narenas = 1;
	parenas = init_arenas;
	memset(parenas, 0, sizeof(arena_t *) * narenas);
	...
	ARENA_SET(parenas[0]);
	parenas[0]->nthreads++;
	/* END BLOCK (plib_initialized == false) */
	} else if ((O_WRONLY|O_RDWR) & fcntl(*swap_fds, F_GETFL)) {
		unsigned i;
		for(i = 0; i < narenas; i++) {
			if (parenas[i] != NULL) {
				parenas[i]->nthreads = 0;
				if (malloc_mutex_init(&parenas[i]->lock))
					return (true);
			}
		}
		ARENA_SET(parenas[0]);
	...
	parenas = (arena_t **)base_alloc(sizeof(arena_t *) * narenas);
	if (parenas == NULL) {
		malloc_mutex_unlock(&init_lock);
		return (true);
	}
...
jemalloc_prefork(void)
{
	...
	for (i = 0; i < narenas; i++) {
		if (parenas[i] != NULL)
			malloc_mutex_lock(&parenas[i]->lock);
	}
...
jemalloc_postfork(void)
{
	...
	for (i = 0; i < narenas; i++) {
		if (parenas[i] != NULL)
			malloc_mutex_unlock(&parenas[i]->lock);
	}
	malloc_mutex_unlock(&arenas_lock);
}
